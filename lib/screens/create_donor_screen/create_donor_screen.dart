...
class _CreateDonorScreenState extends State<CreateDonorScreen> {
  ...
  ///
  /// Utilise la nouvelle méthode unifiée registerComplete qui :
  /// 1. Valide tous les champs du formulaire
  /// 2. Vérifie la correspondance des mots de passe
  /// 3. Appelle l'API unifiée pour créer user + donneur
  /// 4. Connecte l'utilisateur automatiquement
  /// 5. Navigue vers l'étape suivante
  Future<void> _createDonor() async {
    ...
    try {
      ...
      // Inscription unifiée via nouvel endpoint backend
      final result = await api.registerUnified(
      ...
      if (result != null && result['success'] == true) {
        // Stocker les données d'inscription pour l'étape suivante
        RegistrationDataService().setRegistrationData(
          email: _emailController.text.trim(),
          user: result['user'],
        ...
        final errorMessage =
            result?['error'] ?? 'Erreur inconnue lors de l\'inscription';
